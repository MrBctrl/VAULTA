...
import { Plus, ArrowRightLeft, Wallet, X } from 'lucide-react'
import DashboardLayout from '../components/DashboardLayout.jsx'
import AccountCard from '../components/AccountCard.jsx'
import TransactionRow from '../components/TransactionRow.jsx'
import { getAccounts, getTransactions, fundAccount } from '../../lib/api.js'
import { formatConverted, formatCurrency } from '../../lib/currency.js'
...
export default function Accounts() {
  ...
  if (loading) {
    return (
      <DashboardLayout title="Accounts" subtitle="Your VAULTA accounts across every currency">
        <p className="text-sm text-slate-400 py-10 text-center">Loading accounts…</p>
      </DashboardLayout>
    )
  }
  ...
  if (!activeAccount) {
    return (
      <DashboardLayout title="Accounts" subtitle="Your VAULTA accounts across every currency">
        <p className="text-sm text-slate-400 py-10 text-center">No accounts found.</p>
      </DashboardLayout>
    )
  }
  ...
  return (
    <DashboardLayout title="Accounts" subtitle="Your VAULTA accounts across every currency">
      <div className="flex items-center justify-between">
        <h2 className="font-display text-xl font-semibold text-navy-700">VAULTA Accounts</h2>
        <button
      ...
      <div className="grid lg:grid-cols-3 gap-6 mt-8">
        ...
        <div className="rounded-card bg-navy-600 text-white p-6">
          <div className="flex items-center gap-2 text-navy-100/70 text-xs">
            <ArrowRightLeft size={14} /> ESTIMATED NET WORTH
          </div>
          <p className="text-[11px] text-navy-100/50 mt-3 leading-relaxed">
            Approximate combined value of all accounts, converted to Naira
            using indicative rates. Not a live exchange rate.
          </p>
          <div className="mt-5 space-y-3">
            {accounts.map((acc) => (
              <div key={acc.id} className="flex items-center justify-between text-sm">
                <span className="text-navy-100/70">{acc.label}</span>
                <span className="num font-medium">{formatConverted(acc.balance, acc.currency, 'NGN')}</span>
              </div>
            ))}
          </div>
        </div>
...
function AddMoneyModal({ accounts, defaultAccountId, onClose, onFunded }) {
  ...
  async function handleSubmit(e) {
    ...
    setError('')
    try {
      await fundAccount({ accountId, amount: Number(amount), description: 'Deposit' })
      await onFunded()
      onClose()
    } catch (err) {
    ...
    <div className="fixed inset-0 bg-navy-900/40 flex items-center justify-center z-50 px-4">
      <div className="w-full max-w-sm rounded-card bg-white p-6 max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="mt-5 space-y-4">
          ...
          >
            {submitting ? 'Adding…' : `Add ${account ? formatCurrency(Number(amount) || 0, account.currency) : ''}`}
          </button>
        </form>
      </div>
    </div>
