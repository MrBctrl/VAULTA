...
import { getBusinessTransactions } from '../../lib/api.js'

const categories = ['All', 'Income', 'Deposit', 'Transfer']
...
export default function BusinessTransactions() {
  ...
  useEffect(() => {
    getBusinessTransactions().then(setTransactions).finally(() => setLoading(false))
  }, [])
  ...
  return (
    <BusinessLayout title="Transactions" subtitle="Every transaction across the business">
          ...
          {categories.map((c) => (
            <button
              ...
              className={`shrink-0 rounded-full px-3.5 py-1.5 text-xs font-semibold whitespace-nowrap transition-colors ${
                category === c ? 'bg-navy-600 text-white' : 'bg-white border border-silver text-slate-600 hover:border-electric-100'
              }`}
            >
              {c}
            </button>
